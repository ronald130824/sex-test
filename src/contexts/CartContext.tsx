...
export function CartProvider({ children }: CartProviderProps) {
  ...
  function addSnackIntoCart(snack: SnackData): void {
    const snackExistentInCart = cart.find(
      (item) => item.name_icon === snack.name_icon && item.id === snack.id,
    )
    ...
    if (snackExistentInCart) {
      ...
      toast.success(`Outro(a) ${snackEmoji(snack.name_icon)} ${snack.name} adicionado nos pedidos!`)
      saveCart(newCart)
    ...
    toast.success(`${snackEmoji(snack.name_icon)} ${snack.name} adicionado nos pedidos!`)
    saveCart(newCart)
  }

  function removeSnackFromCart(snack: Snack) {
    const newCart = cart.filter((item) => !(item.id === snack.id && item.name_icon === snack.name_icon))
  ...
  function updateSnackQuantity(snack: Snack, newQuantity: number) {
    ...
    const snackExistentInCart = cart.find(
      (item) => item.id === snack.id && item.name_icon === snack.name_icon,
    )

    if (!snackExistentInCart) return

    const newCart = cart.map((item) => {
      if (item.id === snackExistentInCart.id && item.name_icon === snackExistentInCart.name_icon) {
        return {
          ...item,
